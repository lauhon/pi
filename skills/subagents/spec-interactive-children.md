...
| V15 | Without tmux `extended-keys`, Shift+Enter in an attached child submits the prompt (probe: typing `aaa`, Shift+Enter, `bbb` produced a second turn). With `extended-keys on` + `csi-u` at pi startup it adds a newline (no new turn). Turning it on live does **not** fix a pi that is already running: pi picks its key protocol at startup. After `sub kill` + `sub open` on a server that stayed up, the respawned pi gets it. | Same `tmux.conf`. Running children need a restart to pick it up. |
